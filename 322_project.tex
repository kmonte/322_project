...
\title{IDK something about controlling autonomous fleets}
%
%
...
\author{Kyle~Montemayor,
        Ian~Wilmoth,
        and~Willian~Zheng% <-this % stops a space
}
...
\begin{center}
\begin{align}
    \dot{x} &= \frac{r}{2}(u_r+u_l)cos(\theta)   \label{ctrl_x}\\
    \dot{y} &= \frac{r}{2}(u_r+u_l)sin(\theta)   \label{ctrl_y}\\
    \dot{\theta} &= \frac{r}{L}(u_r - u_l)       \label{ctrl_o}
\end{align}
\end{center}

Clearly, the equations for $(\dot{x}, \dot{y})$ are non-linear, but if we define
\begin{equation} \label{ctrl_r}
    \dot{r} = \sqrt{\dot{x}^2 + \dot{y}^2} = \frac{r}{2}(u_r+u_l)
\end{equation}

and use a polar coordinate system originating from the origin of the ADDV 
then we have a completely linear system of equations given by \ref{ctrl_o}, \ref{ctrl_r}.
...
\begin{center}
\begin{align}
    \dot{r} &= \frac{r}{2}(u_r+u_l), u_r = u_l               \label{ctrl_t}\\
    \dot{\theta} &= \frac{r}{L}(u_r - u_l), u_r = -u_l       \label{ctrl_rot}
\end{align}
\end{center}
...
WE SHOULD PROBABLY GO INTO DETAIL ABOUT CONTROLLABILTY AND HOW WE CAN USE THE CONTROL LAWS STRAIGHT UP
